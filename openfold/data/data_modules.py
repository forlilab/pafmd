...
from custom_openfold_utils.conformation_utils import get_residues_ignore_idx_between_af_conformations, get_residues_ignore_idx_between_pdb_af_conformation
...
class OpenFoldSingleDataset(torch.utils.data.Dataset):
    def __init__(self,
                 ...
                 initial_pred_dir: str = None,
                 custom_uniprot_pdb_id: str = None,
                 ):
        """
            Args:
                data_dir:
        ...
        self.initial_pred_dir = initial_pred_dir

        self.supported_exts = [".cif", ".core", ".pdb"]

        valid_modes = ["train", "eval", "predict", "rw_finetuning", "conformation_finetuning"]
        ...
        elif mode == 'conformation_finetuning':
            if custom_uniprot_pdb_id is None:
                uniprot_pdb_ids = [] # list containing all relevant {UniProt_ID/PDB_CHAIN} inside alignment_dir
                uniprot_ids = list(os.listdir(alignment_dir))
                for uid in uniprot_ids:
                    pdb_id = list(os.listdir(os.path.join(data_dir, uid)))[0] #alignment_dir has MSA for pdb_1 and pdb_2, data_dir contains pdb corresponding to the relevant (i.e furthest away) conformation
                    pdb_id = pdb_id.split('.')[0] #remove .pdb
                    uniprot_pdb_ids.append(f"{uid}/{pdb_id}")
                print('ALL UniProt IDs:')
                print(uniprot_pdb_ids)
                self._chain_ids = uniprot_pdb_ids
            else:
                self._chain_ids = [custom_uniprot_pdb_id]
        else:
    ...
    def _parse_mmcif(self, path, file_id, chain_id, alignment_dir, alignment_index, feature_dict=None, custom_template_pdb_id=None, residues_ignore_idx=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'rw_finetuning' or self.mode == 'conformation_finetuning' or self.mode == 'train' or self.mode == 'eval':

            if self.mode == 'conformation_finetuning':
                uniprot_id = name.split('/')[-2]
                file_id, chain_id = (name.split('/')[-1]).split('_')
            else:
                spl = name.rsplit('_', 1)
                if len(spl) == 2:
            ...
            if self.mode == 'rw_finetuning':
                chain_id = 'A' #assumes ground truth conformation arises from a AF-based monomer prediction or cleaned OpenMM structure, in which case chain=A
            ...
            if self.mode == 'conformation_finetuning':
                pdb_path = '%s.pdb' % path
                path = '%s.cif' % path 
                ext = '.cif' #use path (.cif) for training, pdb_path for determining relevant residues to use in loss 
            else:             
            ...
            if self.mode == 'rw_finetuning':
                initial_pred_path = os.path.abspath(glob.glob('%s/*.pdb' % self.initial_pred_dir)[0])
                residues_ignore_idx = get_residues_ignore_idx_between_af_conformations(initial_pred_path, path, initial_pred_path) 
                ...
                if os.path.isfile(features_output_path):
                    ...
                    feature_dict = None
                print('In rw_finetuning mode, ignoring residues for target %s:' % path)
                print(residues_ignore_idx)
            elif self.mode == 'conformation_finetuning':
                initial_pred_path = os.path.abspath(glob.glob('%s/%s/*.pdb' % (self.initial_pred_dir, uniprot_id))[0])
                print('INITIAL PRED PATH: %s' % initial_pred_path)
                residues_ignore_idx = get_residues_ignore_idx_between_pdb_af_conformation(pdb_path, initial_pred_path, initial_pred_path) 
                features_output_path = os.path.join(alignment_dir, 'features.pkl')
                if os.path.isfile(features_output_path):
                    feature_dict = np.load(features_output_path, allow_pickle=True)                  
                else:
                    feature_dict = None
                print('In conformation_finetuning mode, ignoring residues for target %s:' % path)
                print(residues_ignore_idx)
            else:
                feature_dict = None 
            ...
            if ext == ".cif":
                data = self._parse_mmcif(
                    path, file_id, chain_id, alignment_dir, alignment_index, feature_dict, self.custom_template_pdb_id, residues_ignore_idx
                )
            elif ext == ".core":
                ...
                if self.mode == 'conformation_finetuning':
                    n = data["aatype"].shape[-2]
                    data = pad_feature_dict_seq(
                        data, n,
                    )
            else:
...
class OpenFoldSingleMultimerDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        valid_modes = ["train", "eval", "predict", "rw_finetuning"]
        if mode not in valid_modes:
            raise ValueError(f'mode must be one of {valid_modes}')
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'rw_finetuning' or self.mode == 'train' or self.mode == 'eval':
...
class OpenFoldDataModule(pl.LightningDataModule):
    def __init__(self,
                 ...
                 initial_pred_dir: Optional[str] = None,
                 custom_uniprot_pdb_id: Optional[str] = None,
                 **kwargs
                 ):
        ...
        self.custom_template_pdb_id = custom_template_pdb_id 
        self.custom_uniprot_pdb_id = custom_uniprot_pdb_id 
    ...
    def setup(self, stage=None):
        ...
        if self.training_mode:
            if 'rw_finetuning' in self.config_preset: 
                training_mode_type = 'rw_finetuning'
            elif 'conformation_finetuning' in self.config_preset:
                training_mode_type = 'conformation_finetuning'
            else:
            ...
            train_dataset = dataset_gen(
                ...
                initial_pred_dir=self.initial_pred_dir,
                custom_uniprot_pdb_id=self.custom_uniprot_pdb_id,
            )
            ...
            elif training_mode_type == 'conformation_finetuning':
                self.train_dataset = train_dataset 
            elif training_mode_type == 'rw_finetuning':
    ...
    def _gen_dataloader(self, stage):
        ...
        if stage == "train":
            ...
            if training_mode_type not in ['conformation_finetuning', 'rw_finetuning']:
                dataset.reroll()
        elif stage == "eval":
