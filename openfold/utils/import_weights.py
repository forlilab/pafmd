...
def import_jax_weights_(config, model, model_path, no_template=False, version="model_1"):
  
    if config.model.use_chainmask:
        num_chains = config.rw_finetuning.num_chains
        os.makedirs('./openfold/chainmask_params/num_chains=%d' % num_chains, exist_ok=True)
        npz_chainmask_path = './openfold/chainmask_params/num_chains=%d/%s' % (num_chains, model_path.split('/')[-1])
        data = load_jax_weights_chainmask(model_path, npz_chainmask_path, num_chains)
...
def import_openfold_weights_(model, state_dict, config, strict=True):
    """
