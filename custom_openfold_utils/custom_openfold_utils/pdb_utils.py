...
def superimpose_wrapper_monomer(pdb1_full_id: str, pdb2_full_id: str, pdb1_source: str, pdb2_source: str, pdb1_path: str, pdb2_path: str, save_dir: str, TMalign_path: str = None, clean: bool = True, pdb1_chain_id: str = 'A', pdb2_chain_id: str = 'A'):
    ...
    Args:
        ...
        pdb1_source: either 'pdb' or 'pred'
        pdb2_source: either 'pdb' or 'pred' 
    ...
    if 'pred' in pdb1_source:
        logger.info('WARNING: pdb1_source is %s and pdb2_source is %s' % (pdb1_source, pdb2_source))
...
def add_ligand_from_pdb1_to_pdb2(pdb1_id: str, pdb1_path: str, pdb2_path: str, save_dir: str, pdb1_chain_id: str = 'A', pdb2_chain_id: str = 'A', remove_metals: bool = False): 

    if (pdb1_id is not None) and (pdb1_path is None):
        pdb_ref_struc_folder = '%s/pdb_ref_structure' % save_dir
        Path(pdb_ref_struc_folder).mkdir(parents=True, exist_ok=True)
        pdb1_path = fetch_pdb(pdb1_id, pdb_ref_struc_folder)
    ...
    s1 = get_pymol_cmd_superimpose("pdb1", pdb1_chain_id)
    s2 = get_pymol_cmd_superimpose("pdb2", pdb2_chain_id)

    out = cmd.super(s2,s1) #this superimposes s2 onto s1

    #remove nucleic acids and metals 
    cmd.remove("resn A+T+G+C+U")
    #remove additives
    cmd.remove("resn PEG+GOL+EDO+MPD")
    #remove solvent
    cmd.remove("solvent")
    if remove_metals:
        cmd.remove("resn SO4+NA+CL+ZN+FE+CU+MG+CA+MN")

    cmd.select("pdb1_ligand", "pdb1 and not polymer")
    model = cmd.get_model("pdb1_ligand")
    ligand_names = list(set(atom.resn for atom in model.atom))
    print('the following ligands are present in reference %s and will be added to %s' % (pdb1_path, pdb2_path))
    print(ligand_names)

    Path(save_dir).mkdir(parents=True, exist_ok=True)
    pdb2_model_name =  get_model_name(pdb2_path) 
    pdb2_output_path = "%s/%s.pdb" % (save_dir, pdb2_model_name)
    cmd.save(pdb2_output_path, "pdb2 or pdb1_ligand")

    return pdb2_output_path
